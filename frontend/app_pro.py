...
from frontend.views import (
    show_home, show_users, show_products, show_orders, 
...
def main():
    ...
    if selected_page == "🏠 Accueil":
        ...
        auth_service = get_auth_service()
        if auth_service.is_authenticated() and auth_service.is_admin():
            show_page_header("📊 Statistiques", "Tableaux de bord et analyses")
            # show_stats()  # À implémenter
        ...
        auth_service = get_auth_service()
        if auth_service.is_authenticated() and auth_service.is_admin():
            show_page_header("⚙️ Configuration", "Paramètres de l'application")
            # show_config()  # À implémenter
            st.info("🚧 Cette section sera implémentée prochainement.")
        else:
            st.error("🔒 Accès refusé. Cette section est réservée aux administrateurs.")
            st.info("Connectez-vous avec un compte administrateur pour accéder à cette fonctionnalité.")
